--<< Automatically play Nature's Prophet v0.32 (Updated by HiRusSai) >>
...
config:SetParameter("minHealth", 180)
...
local purchaseStartingItems = {12, 45} -- Ring of Protection, Animal Courier
local BuyItem1 = {28,20,14,74} -- Ring of Aquila
local BuyItem2 = {4,16,85,27,16,93,78} -- Mekansm
local BuyItem3 = {8,25,165} -- Maelstorm
local BuyItem4 = {8}
local BuyItem5 = {8}
local BuyItem6 = {167} -- Desolator
local StepsOfBuy={BuyItem1,BuyItem2,BuyItem3,BuyItem4,BuyItem5,BuyItem6}
local StepsOfPrice = {1010,2300,2800,1600,1600,900}
--===================--
--       CODE        --
--===================--
local safe = false
	...
	if PlayingGame() and me.alive then
		...
		if inPosition and state >= 3 and not isAttacking(me) and not me:IsChanneling() then
			...
			if target ~= nil then 
				NotFindTarget = 0
				Minuta = GetMinuts()
				local mekansm = me:FindItem("item_mekansm")
				entityList:GetMyPlayer():Attack(target)
				if me.health <= 350 then
					if mekansm ~= nil then
						if mekansm:CanBeCasted() and me:CanUseItems() then 
							me:CastAbility(mekansm)
						end
					end
					if me:GetAbility(3).level >= 1 and safe == false then 
...
function Key(msg,code)
	...
	if IsKeyDown(config.Test) then
		local me = entityList:GetMyHero()
		client:ExecuteCmd("say_team state = "..state.." inPosition = "..(inPosition and 1 or 0).." TIME = "..client.gameTime.." NumFarmPos = "..NumFarmPos.." NotFindTarget = "..NotFindTarget)
