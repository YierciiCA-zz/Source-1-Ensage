	...
	if msg == KEY_UP then
		...
		elseif code == hotkey2 then
			...
			for q,w in ipairs(fams) do
				...
				if spl:CanBeCasted() and not spl.abilityPhase then
					w:SafeCastAbility(spl)
				end
			end
		end
	end
...
function Tick(tick)
	...
	if active then
		...
		if modif then
			...
			if spell:CanBeCasted() and modif.stacks == spell.level+2 then
				...
				if hp ~= 30000 and spell:CanBeCasted() then
					me:SafeCastAbility(spell,enemy)
				end
			end
		end
		...
		for i,v in ipairs(familiars) do
			...
			if v.health*(1+v.dmgResist) <= 0.3*v.maxHealth*(1+v.dmgResist) and spell:CanBeCasted() then
				v:SafeCastAbility(spell)
			end
			if famils[v.handle].enemy and v:FindModifier("modifier_visage_summon_familiars_stone_form_buff") then
				...
				if famils[v.handle].enemy or (modif and modif.stacks == 0) then
					...
					local spell = v:GetAbility(1)
					if spell:CanBeCasted() or (enemy and spell.cd <= shortest/enemy.movespeed) then
						if shortest <= range then
							...
							if famils[v.handle].stop >= tick then
								...
								if famils[v.handle].sleept < tick and distance <= 30 and (not modif or modif.remainingTime <= 1.01) and spell:CanBeCasted() then
									v:SafeCastAbility(spell)
