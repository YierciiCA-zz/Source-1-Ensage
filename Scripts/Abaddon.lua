...
config:SetParameter("ManaSave", true)
config:Load()
				...
				if not Animations.isAttacking(me) then
					...
					if not shadowplay and victim.alive and victim.visible then
						if W and W:CanBeCasted() and me:CanCast() then
							if config.ManaSave and not me:DoesHaveModifier("modifier_abaddon_borrowed_time") then
								table.insert(castQueue,{1000+math.ceil(W:FindCastPoint()*1000),W,me})
							elseif not config.ManaSave then
								table.insert(castQueue,{1000+math.ceil(W:FindCastPoint()*1000),W,me})
							end
						end
						...
						if abyssal and abyssal:CanBeCasted() and not victim:DoesHaveModifier("modifier_item_sheepstick") and not victim:IsStunned() then
							table.insert(castQueue,{math.ceil(atos:FindCastPoint()*1000),abyssal,victim})
						end
						...
						if atos and atos:CanBeCasted() and me:CanCast() then
							table.insert(castQueue,{math.ceil(atos:FindCastPoint()*1000),atos,victim})
						end
						if sheep and sheep:CanBeCasted() and me:CanCast() then
							table.insert(castQueue,{math.ceil(sheep:FindCastPoint()*1000),sheep,victim})
						end
						if halberd and halberd:CanBeCasted() and distance < halberd.castRange and not victim:IsStunned() and not victim:DoesHaveModifier("modifier_item_sheepstick") then
						...
						if refresher and refresher:CanBeCasted() and me.mana < 375 and me.mana > 225 and soulring and soulring:CanBeCasted() then
							table.insert(castQueue,{100,soulring})
						end
						if refresher and refresher:CanBeCasted() and R and R.cd > 5 and R.state == LuaEntityAbility.STATE_COOLDOWN and me.health > 400 then
							table.insert(castQueue,{100,refresher})
						end
						-- S+R end
						if satanic and satanic:CanBeCasted() and me.health/me.maxHealth <= 0.4 and distance <= attackRange+50 and R and R.state == LuaEntityAbility.STATE_COOLDOWN and not me:DoesHaveModifier("modifier_abaddon_borrowed_time") then
...
function Load()
	if PlayingGame() then
		local me = entityList:GetMyHero()
		if me.classId ~= CDOTA_Unit_Hero_Abaddon then 
			script:Disable() 
		else
			play = true
			victim = nil
