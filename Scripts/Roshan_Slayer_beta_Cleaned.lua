--<<Roshan Slayer beta version | Cleaned by HiRusSai>>--
...
local statusText = drawMgr:CreateText((x+1320)*monitor,(y+25)*monitor,0x39B8E3B3,"Roshan slayer: OFF",Font)
...
function Main(tick)
	...
	if Toggled then
		...
		if ItemsB then
			if Step == 0 and not me:FindItem("item_ward_observer") then
		...
		if Step == 1 and roshan and me:GetDistance2D(roshan) > 345 then -- Edit this lane
			me:AttackMove(Vector(4282,-1816,100))
			Step = 2
			Sleep(1720)
		elseif Step == 2 then
			...
			Step = 3
		elseif Step == 3 and roshan and me:GetDistance2D(roshan) < 296 and me:GetDistance2D(Vector(3478,-1969,100)) < 3.5 then -- Edit this lane
			me:Attack(Ward[1])
			me:Stop()
			Step = 1
		end
